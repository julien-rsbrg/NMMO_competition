...
from utils import *
from CompetitiveNmmoEnv.translators import ObservationToObservationUsefull, ActionUsefullToAction
...
obs2obsUsefull = ObservationToObservationUsefull()
actionUsefull2action = ActionUsefullToAction()


class CompetitionNmmoMultiAgentEnv(MultiAgentEnv):
    
    def __init__(self, team_based_env : TeamBasedEnv):
        self.team_based_env = team_based_env
        self.n_teams = 16
        self.n_soldiers = 8
        n_agents = 128
        self._agent_ids = [str(i) + '_' + str(j) for i in range(self.n_teams) for j in range(self.n_soldiers)]
    ...
    def reset(self, seed = 0) -> dict[AgentID, Obs]:
        ...
        for num_team, observations_by_soldiers in observations_by_team.items():
            for num_soldiers, observation in observations_by_soldiers.items():
                agent_id = str(num_team) + '_' + str(num_soldiers)
                observation = obs2obsUsefull(observation)
                observations_usefull[agent_id] = observation
